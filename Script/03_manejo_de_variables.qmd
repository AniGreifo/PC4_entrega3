...
```{r}
visdat::vis_dat(data_dengue)
```

```{r}
DataExplorer::create_report(data_dengue)
```
...
```{r}
data_dengue_3 = clean_names(data_dengue)
```
...
```{r}
data_dengue_3 = remove_empty(data_dengue_3, which = c("rows", "cols"))
```
